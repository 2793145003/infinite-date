...
import { PhoneShellV2 } from './components/PhoneShellV2';
import { BootScreen } from './pages/Boot';
import { DesktopV2 } from './pages/DesktopV2';
...
export default function AppV2() {
  ...
  const renderScreen = () => {
    ...
    if (!player) {
      ...
      return <div className="id-fade-boot id-fade-boot-v3" key="boot"><BootScreen onLogin={handleLogin} /></div>;
    }
    ...
    switch (view.type) {
      case 'desktop':
        ...
        break;
      case 'character-hub':
        content = <CharacterHub onBack={() => setView({ type: 'desktop' })} onNavigate={setView} />;
        break;
      case 'creation':
        content = <CreatorApp onBack={() => setView({ type: 'character-hub' })} />;
        break;
      case 'diary':
        content = <DiaryApp onBack={() => setView({ type: 'desktop' })} onNavigate={setView} />;
        break;
      case 'sms':
      ...
      case 'mail-detail':
        ...
        break;
      case 'settings':
        content = <SettingsApp player={player} onBack={() => setView({ type: 'desktop' })} onLogout={handleLogout} onUpdate={refreshPlayer} onNavigate={setView} />;
        break;
      case 'facts':
