            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
                ...
                {loc.homeResidents.length > 0 && (
                  <span style={{ fontSize: '0.65rem', color: 'var(--amber)', border: '1px solid var(--amber)', borderRadius: '3px', padding: '0 0.2rem' }}>🏠{loc.homeResidents.map(r => r.name).join('、')}</span>
                )}
            ...
            {editing.homeResidents.length > 0 && (
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.3rem', marginBottom: '0.4rem' }}>
                {editing.homeResidents.map(r => (
                  <span key={r.characterId} style={{ fontSize: '0.72rem', color: 'var(--amber)', border: '1px solid var(--amber)', borderRadius: '3px', padding: '0.15rem 0.3rem', display: 'inline-flex', alignItems: 'center', gap: '0.2rem' }}>
                    {r.name}
                    <button onClick={async () => {
                      try { await api.adminRemoveHome(editing.id, r.characterId); showMsg(`已移除${r.name}的家`); await load(); await refreshEditing(); }
                      catch { showMsg('操作失败'); }
                    }} style={{ background: 'none', border: 'none', color: 'var(--amber)', cursor: 'pointer', padding: 0, fontSize: '0.9rem', lineHeight: 1 }}>×</button>
                  </span>
                ))}
              </div>
            )}
