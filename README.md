...
| 生图 | Krea 2 Turbo（独立容器） | flow-matching 文生图，gemma 扩写英文 prompt，配图/头像/背景图共用 |
...
- **位面任务系统**：玩家去其他位面接委托，遇到「位面崽」（恋爱智能体）。任务大厅 swipe 卡片流 + 位面崽建卡（可发布进公共池）+ 复用场景引擎聊天（动描 + 好感度/目标判定）+ 完成发权限奖励。替代旧世界任务 / NPC 任务（卦象纯函数保留待用）
...
- **互动小说共写引擎**：AI 写世界、玩家写主角（第三人称作者视角接力写），流式续写 + 润色独立 + 出场名单可控（点暗的角色「有事不在」）+ 三折叠上下文（热窗/段摘要/故事总览）+ 世界状态抽取（填表法 + 玩家锚定防时段漂移）
...
- **AI 生图（Krea 2）**：gemma 当摄影师判画面是否出人/出脸（has_person/has_face），配图放开出人，头像按性别锚定称呼
- **头像系统**：玩家头像 + 角色头像（性别锚定），角色档案页头像与聊天页一致；无图时文字头像兜底
- **内驱力（drive）**：角色内驱力字段——玩家可填可生成，没填才 auto-extract（纯规则提炼角色卡）
- **人设人称统一胶囊**：人设叙述字段的 {{角色名}}/{{玩家}} 胶囊 chip，编辑 chip 渲染 + 档案页展开真实名
- **首页每日寄语**：按角色每日一句（场景锚 + 关系现状）
...
│           ├── components/   # 49 个页面/弹窗组件（含互动小说 NovelList/NovelEditor/NovelPlay）
│           └── lib/          # API 客户端 + 主题 + 地图几何
└── pnpm-workspace.yaml
```
...
玩家浏览器（3001/v4）
    ...
    │                                      │
    │                                      ├── 位面任务系统
    │                                      │   ├── 位面崽（恋爱智能体，UGC 公共池，独立数据模型）
    │                                      │   ├── 复用场景引擎聊天（动描 + 好感度/目标判定）
    │                                      │   └── 完成发权限 → 玩家获得奖励
    │                                      │
    │                                      ├── 互动小说共写引擎
    │                                      │   ├── 玩家写主角、AI 写世界（第三人称接力写）
    │                                      │   ├── 润色（独立功能）+ 出场名单（点暗=有事不在）
    │                                      │   └── 三折叠上下文（热窗/段摘要/故事总览）
    │                                      │
    │                                      ├── 三层记忆
    │                                      │   ├── 热窗（近 N 轮原文）
    ...
    └── Krea 2 生图容器（独立，8080 穿透）
        └── flow-matching 文生图（gemma 扩写 prompt → /generate）
```
...
已实现的核心循环：角色创建 → 约会 → 短信 → 记忆 → 主动消息 → 朋友圈 → 场景剧本 → 位面任务（任务大厅 + 位面崽 + 完成发权限）→ 互动小说共写。
