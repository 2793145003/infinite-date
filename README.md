...
- **互动小说共写引擎**：AI 写世界、玩家写主角（第三人称作者视角接力写），流式续写 + 润色独立 + 出场名单可控（点暗的角色「有事不在」）+ 三折叠上下文（热窗/段摘要/故事总览）
...
- **头像系统**：玩家头像 + 角色头像（性别锚定），角色档案页头像与聊天页一致
...
已实现的核心循环：角色创建 → 约会 → 短信 → 记忆 → 主动消息 → 朋友圈 → 场景剧本 → 卦象世界任务（含评级发权限）→ 互动小说共写 → NPC 任务邀请（同行 coop / NPC 独自完成 solo）。
