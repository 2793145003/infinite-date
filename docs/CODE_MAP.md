...
| `scene-rollback.ts` | 338 | **场景约会回滚**。基线快照 + 轮快照滚动窗口 | `rollbackScene`, `captureStartSnapshot`, `captureRoundSnapshot` |
| `scene-end.ts` | 182 | 场景约会结束收尾（折叠记忆 + 清零意愿 + 60%发朋友圈） | `endSceneSession` |
| `scene-map.ts` | 186 | 新地图地点表（scene_locations / scene_homes） | `ensureSceneMap`, `getNpcs`, `upsertNpc`, `getLocationBackground` |
...
| `turn-memory.ts` | 489 | **三层记忆折叠**。热窗(N=5) → 中期折叠(I=12,M=15) → 长期总览 | `foldTurnSegment`, `retrieveTurnMemory`, `runTurnMemoryUpdate`, `foldDateSummary` |
| `memory.ts` | 916 | **旧记忆系统**。Chronicle 折叠 + Player Facts（短信/老约会/群聊用） | `foldChronicle`, `retrieveRelevantMemories`, `maybeFoldIncremental` |
...
| `schedule.ts` | 870 | **NPC行程系统**。deterministic hash 生成，按性格分模板池 | `getSceneSchedule`, `getCurrentSchedule`, `classifyPersonality` |
| `moment-scheduler.ts` | 46 | **后台行程驱动**。5min 扫一次，不依赖玩家在线 | `startMomentScheduler` |
| `proactive.ts` | 312 | **NPC主动消息**。意愿累积机制（sms_urge/moment_urge）+ 行程变更检测 | `checkScheduleChange`, `resetSmsUrge`, `resetMomentUrge`, `clearUrgeAfterDate`, `initUrge`, `getUnansweredProactiveCount` |
| `presence.ts` | 278 | **玩家在线状态 + 旧系统NPC主动消息**。15s 心跳 | `updatePresence`, `checkProactive` |
...
| 文件 | Method | Path | 功能 |
|---|---|---|---|
| **scene.ts** | GET | `/scene/locations` | 场景地图地点列表 |
...
## 三、前端页面（apps/web/src/pages/）

> 前端无 React Router。`App.tsx` 用 `View` 类型状态机导航：`setView({ type: 'xxx' })`。

| 文件 | 行数 | View type | 功能 |
|---|---|---|---|
| `Desktop.tsx` | 173 | `desktop` | 手机桌面（九宫格入口，旧剧本已移至回收站） |
| `SmsApp.tsx` | 874 | `sms` / `sms-thread` | 短信（列表+对话） |
| `SceneMapApp.tsx` | 206 | `scenemap` | 新地图（地点树+谁在哪） |
| `SceneLocation.tsx` | 593 | `scene-location` | 地点详情（NPC列表+邀请+探索入口） |
| `SceneConversation.tsx` | 612 | `scene-conversation` | **场景约会对话**（SSE气泡+心声+撤回+重试） |
| `SceneExplore.tsx` | 250 | `scene-explore` | 场景探索（逛逛/偶遇/上前） |
| `MapApp.tsx` | 316 | `map` | 旧地图 |
| `LocationDetail.tsx` | 566 | `location-detail` | 旧地点详情 |
| `Conversation.tsx` | 491 | `conversation` / `group-conversation` | 旧约会对话 |
| `Explore.tsx` | 153 | `explore` | 旧探索 |
| `MissionsApp.tsx` | 361 | `missions` | 任务系统 |
| `ScenarioList.tsx` | 128 | `scenarios` | 剧本列表 |
| `ScenarioDetail.tsx` | 262 | `scenario-detail` | 剧本详情 |
...
| `MySpaceApp.tsx` | 308 | `myspace` | 我的空间 |
| `SettingsApp.tsx` | 305 | `settings` | 设置 |
| `FeedbackApp.tsx` | 623 | `feedback` | 反馈+更新日志 |
| `MailApp.tsx` | 98 | `mail` / `mail-detail` | 邮件 |
| `ArchiveApp.tsx` | 547 | `archive` | 回忆（场景约会+场景剧本页签，旧记录折叠） |
| `ArchivedApps.tsx` | 67 | `archived` | 回收站（旧地图+旧剧本，只读） |
| `AdminApp.tsx` | 39 | `admin` | 管理后台 |
| `Boot.tsx` | 101 | — | 启动页（登录检查） |
| `Login.tsx` | 44 | — | 登录页 |
| `FishMode.tsx` | 145 | — | 钓鱼模式 |

### 前端 lib

| 文件 | 行数 | 功能 |
|---|---|---|
| `api.ts` | 1599 | 后端 API 客户端（fetch + SSE stream + token 管理） |
| `themes.ts` | 81 | 主题（QQ浏览器不支持 color-mix → CSS 变量 --overlay-bg） |
| `usePresence.ts` | 88 | 玩家在线心跳（15s POST /presence） |
| `text-render.tsx` | 37 | 文本渲染（@提及等） |
...
| `AvatarCropModal.tsx` | 136 | 头像裁剪（只裁头像，不过度推广） |
| `CharacterEditModal.tsx` | 357 | 角色编辑弹窗 |
| `ImageUploadButton.tsx` | 143 | 图片上传按钮 |
| `AutoTextarea.tsx` | 26 | 自动高度文本框 |
| `LiveConflictModal.tsx` | 128 | 会话冲突弹窗 |
